...
| How does a customer authorize their OEM (or CSP) partner to register Autopilot devices on their behalf?    |  Within Microsoft Partner Center (MPC), an OEM or CSP will find a link called "request a reseller relationship" on the Customer page.  Clicking that link exposes an email template the OEM or CSP can send to the customer.  The email contains a hyperlink to Microsoft Store for Business (MSfB).  When a customer with a MSfB (or Intune) global admin account clicks the link, they will be taken directly to a page in MSfB where they can click the "accept" button to grant the OEM or CSP authorization.  They do NOT also have to grant the OEM or CSP delegated admin rights to their account; delegated admin rights can be removed either by the OEM (or CSP) in MPC, or by the customer via the MSfB.  After clicking "accept", authorization is instantly granted to the OEM (or CSP) and the customer should appear in the OEM's customer list within MPC within minutes. |
